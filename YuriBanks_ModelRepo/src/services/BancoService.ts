...
export default class BancoService {
  ...
  criarConta(titular: string, a?: number | TipoConta, b?: number): ContaBase {
    ...
    if (typeof a === 'number') {
      ...
      tipo = a;
      saldoInicial = typeof b === 'number' ? b : 0;
    }

    const conta: ContaBase = tipo === 'poupanca'
      ? new ContaPoupanca({ titular, saldoInicial })
      : new ContaCorrente({ titular, saldoInicial });
  ...
  depositar(numero: number, valor: number, descricao?: string): void {
    const conta = this.mustGet(numero);
    // usar sobrecarga
    if (descricao) conta.depositar(valor, descricao); else conta.depositar(valor);
    this.repo.save(conta); // persistir estado
  }
  ...
  transferir(origem: number, destino: number, valor: number): void {
    ...
    const cOrigem = this.mustGet(origem);
    const cDestino = this.mustGet(destino);
    cOrigem.enviarTransferencia(valor, cDestino.numero!);
    cDestino.receberTransferencia(valor, cOrigem.numero!);
    this.repo.save(cOrigem);
    this.repo.save(cDestino);
  }
  ...
  processarCicloMensal(): void {
    for (const c of this.repo.findAll()) {
      c.processarCicloMensal();
      this.repo.save(c);
    }
  }

  private mustGet(numero: number): ContaBase {
    const c = this.repo.findById(numero);
    if (!c) throw new Error('Conta nao encontrada.');
    return c;
    }
}
