...
export default class ContasMemRepository implements IContaRepository {
  ...
  save(entity: ContaBase): ContaBase {
    if (entity.numero == null) {
      entity.numero = this.nextNumero();
    }
    this.store.set(entity.numero, entity);
    return entity;
  }
  ...
  findByTitular(nome: string): ContaBase[] {
    ...
    return this.findAll().filter(c => c.titular.toLowerCase().includes(t));
  }
}
